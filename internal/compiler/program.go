...
func NewProgram(options ProgramOptions) *Program {
	...
	p.host.Trace("before processAllProgramFiles")
	p.processedFiles = processAllProgramFiles(p.host, p.programOptions, p.compilerOptions, rootFiles, libs, p.projectReferences, p.singleThreaded())
	p.host.Trace("aftert processAllProgramFiles")
